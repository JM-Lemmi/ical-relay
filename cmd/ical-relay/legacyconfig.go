...
type profileV2 struct {
	...
	Tokens        []string         `yaml:"admin-tokens"`
	NTokens       []database.Token `yaml:"admin-tokens-storage-v2,omitempty"`
	Rules         []database.Rule  `yaml:"rules,omitempty"`
}
...
func LegacyParseConfig(path string) (Config, error) {
	...
	if configVersion.Version == 2 {
		...
		tmpConfig.Version = 3
		tmpConfig.Profiles = make(map[string]database.Profile)
		...
		for p := range configV2.Profiles {
			...
			tmpConfig.Profiles[p] = database.Profile{
