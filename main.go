...
func main() {
	...
	router.HandleFunc("/{profile}/monthly", monthlyViewHandler).Name("monthlyView")
	router.HandleFunc("/{profile}/edit/{uid}", editViewHandler).Name("editView")
	router.HandleFunc("/profiles/{profile}", profileHandler).Name("profile")
	router.HandleFunc("/login", loginHandler).Name("login")
